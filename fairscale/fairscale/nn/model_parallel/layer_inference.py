...
class TernaryColumnParallelLinear(torch.nn.Module):
    """Linear layer with column parallelism.
    ...
    def __init__(
        ...
        super(TernaryColumnParallelLinear, self).__init__()
        ...
        # Note: torch.nn.functional.linear performs XA^T + b and as a result
        # we allocate the transpose.
        self.register_buffer('weight', torch.zeros((self.output_size_per_partition // 4, self.in_features), dtype=torch.uint8))
        self.alphas = Parameter(torch.randn(1))
        if bias:
            self.bias = Parameter(torch.Tensor(self.output_size_per_partition))
            # Always initialize bias to zero.
            with torch.no_grad():
    ...
    def get_ternary_weights(self):
        ...
        # return ternary.apply(self.weight)
        return hqq_aten.unpack_2bit_u8(self.weight).to(torch.float) - 1.0

    def forward(self, input_: torch.Tensor) -> torch.Tensor:  # type: ignore
        # Set up backprop all-reduce.
        input_parallel = copy_to_model_parallel_region(input_)
        # Matrix multiply.
        w = self.alphas * self.get_ternary_weights()
        # output_parallel = F.linear(input_parallel, self.weight, self.bias)
        output_parallel = F.linear(input_parallel, w, self.bias)
        if self.gather_output:
            # All-gather across the partitions.
            output = gather_from_model_parallel_region(output_parallel)
        else:
...
class TernaryRowParallelLinear(torch.nn.Module):
    """Linear layer with row parallelism.
    ...
    def __init__(
        ...
        super(TernaryRowParallelLinear, self).__init__()
        ...
        # Note: torch.nn.functional.linear performs XA^T + b and as a result
        # we allocate the transpose.
        self.register_buffer('weight', torch.zeros((self.out_features // 4, self.input_size_per_partition), dtype=torch.uint8))
        self.alphas = Parameter(torch.randn(1))
        if bias:
            self.bias = Parameter(torch.Tensor(self.out_features))
            # Always initialize bias to zero.
